...
def insertCustHierarchyTask(spark, logger, g11_db_name, target_db_name, target_table):
    ...
    # Persist and validate critical columns
    cust_hierarchy656_na_lkp_df.persist(StorageLevel.MEMORY_AND_DISK)

    null_count = cust_hierarchy656_na_lkp_df.filter("customer_id IS NULL OR l1_global_lvl IS NULL").count()
    if null_count > 0:
        logger.warning(f"Found {null_count} rows with missing customer_id or l1_global_lvl.")
