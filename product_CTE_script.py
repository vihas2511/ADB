...
def insertProdNaDimTask(spark, logger, hive_database, target_db_name, target_table):
    # We switch the current database first.
    spark.sql(f"USE {hive_database}")
    logger.info(f"Switched to database: {hive_database}")

    # Instead of creating temporary views separately, we "inline" all the logic
    # with a WITH clause (CTE). This does not persist any view in the catalog.
    query = f"""
    WITH
    ...
    tmp_ztxxptmelf_stage_life_cycle AS (
      ...
      FROM (
        ...
        FROM (
          ...
          FROM ${hivevar:dbPrdMasterDataG11}.ZTXXPTMELF
          WHERE ZLCS IN ('US', 'CA')
        ) a
        GROUP BY a.material_number
      ) b
    ...
    tmp_ztxxptatva_attribute AS (
      ...
      FROM (
        ...
        FROM (
          ...
          FROM ${hivevar:dbPrdMasterDataG11}.ZTXXPTATVA
    ...
    tmp_marm_it AS (
      ...
      WHERE MEINH = 'IT' 
        AND EXISTS (
          ...
          WHERE ${hivevar:dbPrdMasterDataG11}.MARM.MATNR = mat.material_number
        )
    ),
    tmp_marm_sw AS (
      SELECT
    ...
    tmp_ztxxpvclsf_1 AS (
      ...
      FROM (SELECT ZPICKID FROM ${hivevar:dbPrdMasterDataG11}.ZTXXPTCPIK WHERE ZPICDES = 'LANGUAGE ON PACK') AS pik
      JOIN ${hivevar:dbPrdMasterDataG11}.CABN AS cabn
    ...
    tmp_ztxxpvclsf_ausp AS (
      ...
      FROM ${hivevar:dbPrdMasterDataG11}.AUSP AS ausp
      WHERE EXISTS (
    ...
    tmp_ztxxpvclsf_kssk AS (
      ...
      FROM ${hivevar:dbPrdMasterDataG11}.KSSK AS kssk
      WHERE EXISTS (
    ...
    tmp_ztxxpvclsf AS (
      ...
      JOIN ${hivevar:dbPrdMasterDataG11}.KSML AS ksml
      ...
      JOIN ${hivevar:dbPrdMasterDataG11}.KLAH AS klah
    ...
    tmp_gocoa_planning_unit AS (
      ...
      FROM (
        ...
        FROM ${hivevar:dbPrdMasterDataG11}.ZTXX_PTWFREQ
    ...
    spark.sql(query)
    logger.info(f"Data has been successfully loaded into {target_db_name}.{target_table}")
    return 0
...
def main():
    ...
    target_table = config['tables']['prod1_na_dim']
